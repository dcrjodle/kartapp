 ...
 */
export const calculateGeographicBounds = (municipalities: Municipality[]): Bounds => {
...
export const projectToSVG = (
  ...
  const y = ((maxMercatorY - mercatorY) / (maxMercatorY - minMercatorY)) * dimensions.height;
 ...
 */
export const generateMeridians = (bounds: Bounds, dimensions: MapDimensions, interval: number = 2) => {
  const meridians = [];
 ...
 */
export const generateParallels = (bounds: Bounds, dimensions: MapDimensions, interval: number = 2) => {
  const parallels = [];
